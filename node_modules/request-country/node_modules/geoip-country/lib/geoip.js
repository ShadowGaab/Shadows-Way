...
function preload(callback) {
	...
	if (typeof arguments[0] === 'function') {
		async.series([
				...
				async.series([
					...
					function (cb2) {
						fs.fstat(datFile, function (err, stats) {
							datSize = stats.size;
							asyncCache.locationBuffer = Buffer.alloc ? Buffer.alloc(datSize) : new Buffer(datSize);
							cb2(err);
						});
					},
					function (cb2) {
			...
			function () {
				asyncCache.mainBuffer = Buffer.alloc ? Buffer.alloc(datSize) : new Buffer(datSize);
		...
		cache4.mainBuffer = Buffer.alloc ? Buffer.alloc(datSize) : new Buffer(datSize);
...
function preload6(callback) {
	...
	if (typeof arguments[0] === 'function') {
		async.series([
			...
			function () {
				asyncCache6.mainBuffer = Buffer.alloc ? Buffer.alloc(datSize) : new Buffer(datSize);
		...
		cache6.mainBuffer = Buffer.alloc ? Buffer.alloc(datSize) : new Buffer(datSize);
